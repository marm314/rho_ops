...
Input::Input(string rho_in)
{
 ...
 dmn_thresh=false;esi_int=false;dmn_indicators=false;nopath=true;dim2=false;dim3=false;quadrature=false;becke=false;tfvc=false;
 ...
 if(rho_input_file.good()) //Check existence of file
 {
  while(getline(rho_input_file,rho_in))
  {
   ...
   else if(rho_in=="$integrals")
   {
    ...
    else //Becke or TFVC Quadrature
    {
     if(cuba_cubature=="becke"){becke=true;}
     else{tfvc=true;}
   ...
   else if(rho_in=="$mescal")
   {
    ...
    mescal_pdb=rho_in;
    rho_input_file>>maxiter_mescal>>thresh_mescal_mu>>thresh_mescal_E>>r0_mescal>>w_mu_mescal;
   }
