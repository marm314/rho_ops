 ...
 if(my_chk.good()) //Check existence of file
 {
  ...
  if(Input_commands.punctualr)
  {
   ...
   for(i=0;i<Input_commands.punctuals_r;i++)
   {
    ...
    ID_IND_local(Read_fchk_wfn,Point,ID_alpha,ID_beta,IND_alpha,IND_beta,firstcall);
  ...
  if(Input_commands.integrals)
  {
   ...
   if(Input_commands.cuba) //Cuba
   {
    ...
    if(sij)
    {
     ...
     for(i=0;i<Input_commands.nregions;i++)
     {
      ...
      if(mos_to_nos_dmn)
      {
       mos_to_nos_dmn_sij(SIJ,Read_fchk_wfn,Input_commands,name_file,Read_fchk_wfn.wfn);
      }
      string name_tmp="";
      if(Read_fchk_wfn.wfn)
      {
       name_tmp=name_file.substr(0,(name_file.length()-4))+"_X"+region_string+".int";
      }
      else
      {
       name_tmp=name_file.substr(0,(name_file.length()-5))+"_X"+region_string+".int";
      }
      print_int(Read_fchk_wfn,name_tmp,SIJ,Read_fchk_wfn.nbasis(),Density,rhoa,rhob,region_string);
   ...
   else if(Input_commands.quadrature) 
   {
    ...
    if(sij || r1_moment)
    {
     ...
     {
      mos_to_nos_int_fchk_dm1(Read_fchk_wfn,Input_commands,name_file,ORBITALS,total_grid,nbasis,wfn_fchk);
     }
     for(num_integrals=0;num_integrals<Input_commands.ops;num_integrals++)
     {
      if(Input_commands.integral_ops[num_integrals]=="sij")
      {
       ...
       print_int(Read_fchk_wfn,name_tmp,Sij,nbasis,Density,rhoa,rhob,region_string);
       Results<<name_tmp<<endl;
       region_int++;
      }
