...
void READ_FCHK_WFN::rho_eval(double point[3],double &result)
{
 ...
 else
 {
  ...
  for(i[0]=0;i[0]<MO_coef;i[0]++)
  {
   ...
   {
    build_NO_grad_wfn(result,grad,point,i[0]);
    density=density+result*result*Ocupation[i[0]];
   }
  }
  //Return result
  result=density;
  ...
  else
  {
   ...
   if(!correlated)
   {
    for(i[0]=0;i[0]<MO_coef;i[0]++)
    {
     if(Ocupation[i[0]]!=ZERO)
     {
      build_NO_grad_wfn(result[0],grad,point,i[0]);
      if(i[0]%2==0)
      {
       rhoa=rhoa+result[0]*result[0]*Ocupation[i[0]]; //calculate rhoa
      }
      else
      {
       rhob=rhob+result[0]*result[0]*Ocupation[i[0]]; //calculate rhob
      }
     }
    }
   }
...
void READ_FCHK_WFN::rho_grad(double point[3],double Grad[3])
{
 ...
 {
  double *MOs,**MO_grad,*temp,*temp1;
  ///////////////////////////////////////
  //dynamic arrays
  temp=new double[1];temp1=new double[3];
  ...
  for(i[0]=0;i[0]<MO_coef;i[0]++)
  {
   if(Ocupation[i[0]]!=ZERO)
   {
    build_NO_grad_wfn(temp[0],temp1,point,i[0]);
    for(j[0]=0;j[0]<3;j[0]++)
    {MO_grad[j[0]][i[0]]=temp1[j[0]];}
    MOs[i[0]]=temp[0];
   }
  }
   ////////////////////////////////////////////////////////////////////////////
  ...
  else
  {
   ...
   if(!correlated)
   {
    for(i=0;i<MO_coef;i++)
    {
     if(Ocupation[i]!=ZERO)
     {
      build_NO_grad_wfn(result,grad,point,i);
      if(i%2==0)
      {
       for(k=0;k<3;k++)
       {
        Grada[k]=Grada[k]+TWO*result*Ocupation[i]*grad[k]; //calculate grada
       }
      }
      else
      {
       for(k=0;k<3;k++)
       {
        Gradb[k]=Gradb[k]+TWO*result*Ocupation[i]*grad[k]; //calculate grada
       }
      }
     }
    }
...
void READ_FCHK_WFN::orb_grad(double point[3],double **res)
{
 ...
 else
 {
  for(i=0;i<nbasisf;i++)
  {
   build_NO_grad_wfn(evaluation,AUX,point,i);
   res[0][i]=evaluation;
   for(j=0;j<3;j++)
   {res[j+1][i]=AUX[j];}
  }
...
void READ_FCHK_WFN::rho_p_eval(double point_p[3],double &result)
{
 ...
 else //Build for wfn
 {
  ...
  for(i[0]=0;i[0]<MO_coef;i[0]++)
  {
   if(Ocupation[i[0]]!=ZERO)
   {
    build_NOp_wfn(NOp[0],point_p,i[0]);
    density=density+Ocupation[i[0]]*pow(abs(NOp[0]),TWO);
   }
  }
  //Return result
  result=density;
  ...
  else
  {
   ...
   if(!correlated)
   {
    for(i=0;i<MO_coef;i++)
    {
     if(Ocupation[i]!=ZERO)
     {
      build_NOp_wfn(NOp,point_p,i);
      if(i%2==0)
      {
       rhoa=rhoa+Ocupation[i]*pow(abs(NOp),TWO); //calculate rhoa
      }
      else
      {
       rhob=rhob+Ocupation[i]*pow(abs(NOp),TWO); //calculate rhob
      }
     }
    }
   }
 ...
 else //Build for wfn
 {
  ...
  for(k=0;k<3;k++)
  {
   ...
   for(i=0;i<MO_coef;i++)
   {
    if(Ocupation[i]!=ZERO)
    {
     build_NOp_wfn(NOp[0],aux1,i);
     grad_p[k]=grad_p[k]+Ocupation[i]*pow(abs(NOp[0]),TWO);
     build_NOp_wfn(NOp[0],aux2,i);
     grad_p[k]=grad_p[k]-Ocupation[i]*pow(abs(NOp[0]),TWO);
    }
   }
  }
